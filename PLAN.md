...
- `excel-runtime`
  ...
  - arithmetic/comparison/logical/math helper, aggregate/count helper, criteria aggregate, lookup/reference and reference metadata helper, date/time serial/text-parse helper, error/info helper, focused text helper formula subset을 회귀 테스트와 함께 제공한다.
...
- 상태
  ...
  - 수식 엔진은 arithmetic, comparison, logical, math helper, aggregate/count helper, criteria aggregate, lookup/reference and reference metadata helper, date/time serial helper, error/info helper subset을 현재 regression으로 고정했다.
