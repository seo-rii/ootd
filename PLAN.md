...
- `excel-runtime`
  ...
  - arithmetic/comparison/logical/math helper, aggregate/count helper, criteria aggregate, lookup/reference helper, error/info helper formula subset을 회귀 테스트와 함께 제공한다.
...
- 상태
  ...
  - 수식 엔진은 arithmetic, comparison, logical, math helper, aggregate/count helper, criteria aggregate, lookup/reference helper, error/info helper subset을 현재 regression으로 고정했다.
...
- broader formula parity: string/date semantics, additional lookup/reference families, name resolution, richer coercion semantics
