...
- Runtime Range object support preserves explicit multi-area order for address, areas, count, scalar assignment, calculation, find/replace, clear, fill, offset/resize, first-area row/column projections, `Range.Select`/`Application.Selection`, and focused chart source binding. Active-cell fallbacks and copied/cut cell paste materialization still use first-area or single-area boundaries, while clipboard payloads retain `RangeSet` metadata for chart source binding.
