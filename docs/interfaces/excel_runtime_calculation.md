...
- `Formula2` and `Formula2Local` materialize row- or column-filtered `FILTER` results as two-dimensional spill ranges. Recalculation replaces the previous spill extent, clears cells no longer in the result, and returns `#SPILL!` without overwriting an occupied destination cell.
- Other dynamic array projection helpers, including `BYCOL`, `BYROW`, `GROUPBY`, `MAKEARRAY`, `MAP`, `PIVOTBY`, `REDUCE`, `SCAN`, `SORT`, `SORTBY`, and `UNIQUE`, still return the scalar top-left, first projected, or scalar accumulator value. `Formula` retains this scalar compatibility behavior for `FILTER` as well.
...
- Dynamic array support currently covers in-memory `Formula2`/`Formula2Local` `FILTER` spill materialization, spill replacement, and obstruction errors. XLSX dynamic-array metadata round-trip, spill-reference operators, additional array-producing functions, and broader `Formula2` parity remain incomplete.
