...
- `Formula2` and `Formula2Local` materialize two-dimensional spill ranges for `FILTER`, `SORT`, `SORTBY`, `UNIQUE`, `TAKE`, `DROP`, `CHOOSECOLS`, `CHOOSEROWS`, `TRANSPOSE`, and `SEQUENCE`. This includes row/column filtering, row- or column-oriented sorting, multiple `SORTBY` keys, row/column uniqueness, signed selection counts, ordered repeated row/column indexes, and mixed-type source values.
