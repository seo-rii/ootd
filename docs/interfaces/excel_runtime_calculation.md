...
- `Formula2` and `Formula2Local` materialize two-dimensional spill ranges for `FILTER`, `SORT`, `SORTBY`, `UNIQUE`, `TAKE`, `DROP`, `CHOOSECOLS`, `CHOOSEROWS`, `TRANSPOSE`, `SEQUENCE`, `EXPAND`, `HSTACK`, `VSTACK`, `TOCOL`, `TOROW`, `WRAPROWS`, and `WRAPCOLS`. This includes row/column filtering, row- or column-oriented sorting, multiple `SORTBY` keys, row/column uniqueness, signed selection counts, ordered repeated row/column indexes, mixed-type source values, explicit/default padding, and row- or column-major flattening.
- Spill recalculation replaces the previous extent, clears cells no longer in the result, and returns `#SPILL!` without overwriting an occupied destination cell. `Formula` retains the existing scalar compatibility behavior for these functions.
- Other dynamic array projection helpers, including `BYCOL`, `BYROW`, `GROUPBY`, `MAKEARRAY`, `MAP`, `PIVOTBY`, `REDUCE`, and `SCAN`, still return the first projected or scalar accumulator value.
...
- `PY` parses Python-in-Excel formulas and returns deterministic `#BLOCKED!` because this runtime does not execute Microsoft Cloud Python code.
...
- Dynamic array support currently covers in-memory `Formula2`/`Formula2Local` spill materialization for the projection functions listed above, spill replacement, and obstruction errors. XLSX dynamic-array metadata round-trip, spill-reference operators, remaining array-producing and higher-order functions, and broader `Formula2` parity remain incomplete.
