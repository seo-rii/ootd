...
- `VALUE` supports plain decimal numeric text, comma grouping, the invariant `$` currency symbol, accounting parentheses, and trailing percent signs; `NUMBERVALUE` additionally supports configurable decimal and group separators. Date/time text and locale-specific formatting beyond those focused paths are not implemented.
