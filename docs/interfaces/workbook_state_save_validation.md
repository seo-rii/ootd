...
`WorkbookState::rearrange_cells_with_change` owns the final destination batch produced by row- and
column-oriented `Range.Sort`. Runtime computes ordering and every changed destination from one
immutable cell snapshot. The command validates all coordinates and rejects any changed dynamic
spill anchor or materialized/geometric spill child before mutation; allowed plain cells, worksheet
dirty tracking, and runtime semantic dirty state are then committed without a second fallible
workbook lookup. The regressions put a normal changed destination before the `J10` spill anchor in
both orientations (`I10:K11` rows and `J9:K11` columns) and require workbook persistence,
dirty-domain, and session snapshots to remain unchanged.

`WorkbookState::fill_cells_with_change` owns the source boundary and final destination overlay
produced by `Range.FillDown`, `FillRight`, `FillUp`, and `FillLeft`. Runtime stages only touched
destinations over the immutable original cell map, processes multi-area input in declared order,
and lets later overlapping areas read earlier staged results. This preserves existing overlap,
styled-blank, and A1 formula-shift behavior without registering temporary per-area Range handles.
For every nontrivial fill, the command validates every source and destination coordinate before
diff filtering and refuses any dynamic spill anchor or materialized/geometric child, including an
unchanged protected destination. Four-direction destination and source-only regressions, a
multi-area earlier-commit regression, and an overlap-order regression fix the workbook,
dirty-domain, and session contracts.

`WorkbookState::validate_copy_source_cells` and `copy_cells_with_change` form the two-state command
boundary for single-area `Range.Copy Destination`. Runtime snapshots every source cell and validates
the complete source range against its owning workbook before preparing any destination mutation.
It then preserves existing styled blanks, shifts A1 formulas by source-to-target offset, and builds
one replacement map from an immutable destination view. The destination command validates every
coordinate and refuses any dynamic spill anchor or materialized/geometric child before diff
filtering and commit. This split supports same-sheet, cross-sheet, and cross-workbook copies without
mutating the source. Regressions cover a normal destination before a spill anchor, spill anchor and
child sources, and cross-workbook destination failure while preserving both workbooks, dirty
domains, and seeded Find/clipboard session state.

`WorkbookState::validate_cut_source_cells` and `cut_cells_with_change` extend that strict transfer
boundary to single-area `Range.Cut Destination`. Runtime snapshots source cells once, builds both
the destination replacement map and style-preserving source-clear map from immutable worksheet
views, and executes every fallible command against prepared `WorkbookState` clones. A same-workbook
cut swaps in one prepared state only after destination and source commands succeed; a cross-workbook
cut prepares both states before either live owner is replaced. Same-sheet overlap excludes target
coordinates from the clear map so destination values win, while moved formulas keep their exact
text and cache instead of receiving Copy's A1 offset. Source and destination validation includes
unchanged coordinates and rejects dynamic spill anchors and materialized/geometric children.
Regressions cover spill failures after an earlier normal target, anchor/child sources,
cross-sheet/cross-workbook rollback, exact session preservation through default Cut-mode
...
column limit. Before corridor planning, the command inventories cell-formula and defined-name
owners across the workbook. Any R1C1 owner, or any A1 owner whose common lexical detector finds a
bounded cell or whole-axis reference outside a quoted string, returns stable `Unsupported` before
mutation. Defined-name diagnostics include the workbook/worksheet scope and display name. The check
is deliberately workbook-wide until a common reference AST can resolve owner sheet and target
...
intersections, unmaterialized-child failures, and reference-owner preflight failures require exact
...
immutable-snapshot, spill-aware cell batch, and stage fourteen gives `Range.Sort` a
spill-refusing rearrangement batch for both orientations. Stage fifteen moves all four directional
Fill methods to one ordered overlay and source/destination-preflighted model commit. Stage sixteen
