...
completed `capture_manifest.json`의 `writableOutputs`는 이 5개 payload에 대응하는 canonical logical key set만 포함해야 하며, `office-codegen`은 예상 외 writable output key도 거부한다.
