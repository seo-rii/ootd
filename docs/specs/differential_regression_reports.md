...
directory, and rejects symlinked output roots plus canonical artifact paths that
are already directories or symlinks, so context mismatch or path conflicts
cannot leave a report-only partial artifact or follow artifact links outside
the output root.
