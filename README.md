...
- `excel-runtime` formula evaluation for arithmetic, comparisons, logical/control helpers, aggregates and statistical rank helpers, criteria aggregates, lookup/reference and reference metadata helpers, date/time serial, week, and text-parse helpers, error/info helpers, focused text helpers, and `Application`/`Worksheet.Evaluate`,
