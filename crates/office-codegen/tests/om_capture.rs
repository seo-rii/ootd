...
fn source_registry_rejects_malformed_identifier_fields() {
    ...
    for (malformed_toml, expected_message) in [
        ...
        (
            registry_toml
                .replace(
                    "[test_corpus.official_ms]\nenabled = true",
                    "[test_corpus.official_ms]\nenabled = false",
                ),
            "source registry official_ms corpus had enabled sources while disabled",
        ),
        (
            registry_toml
                .replace(
                    "[test_corpus.open_source]\nenabled = true",
                    "[test_corpus.open_source]\nenabled = false",
                ),
            "source registry open_source corpus had enabled sources while disabled",
        ),
        (
            registry_toml
                .replace(
            ...
            "source registry official_ms corpus was enabled without enabled sources",
        ),
        (
            registry_toml
                .replace("open_xml_sdk = true", "open_xml_sdk = false")
            ...
            "source registry open_source corpus was enabled without enabled sources",
        ),
        (
            registry_toml
                .replace(
...
#[cfg(unix)]
#[test]
fn output_root_writer_rejects_parent_symlink_before_partial_write() {
    let registry_toml =
        fs::read_to_string(repo_root().join("specs/sources.toml")).expect("source registry");
    let source_summary = summarize_source_registry_toml(&registry_toml).expect("source summary");
    let report = build_differential_report_with_source_context(
    ...
    let unique_suffix = SystemTime::now()
        ...
        .as_nanos();
    let symlink_target = std::env::temp_dir().join(format!(
        "ootd-differential-output-root-parent-symlink-target-{unique_suffix}"
    ));
    let symlink_parent = std::env::temp_dir().join(format!(
        "ootd-differential-output-root-parent-symlink-{unique_suffix}"
    ));
    fs::create_dir_all(&symlink_target).expect("output root parent symlink target");
    std::os::unix::fs::symlink(&symlink_target, &symlink_parent)
        .expect("output root parent symlink");
    let output_root = symlink_parent.join("artifacts");
    let target_paths = differential_artifact_paths(symlink_target.join("artifacts"));

    let error =
        write_differential_report_and_gate_to_output_root(&report, &source_summary, &output_root)
            .expect_err("output root parent symlink should fail before write");

    match error {
        DifferentialReportLoadError::Contract { message } => {
            assert!(message.contains("differential output root parent path"));
            assert!(message.contains("was a symlink"));
        }
        other => panic!("unexpected error: {other:?}"),
    }
    assert!(!target_paths.report_path.exists());
    assert!(!target_paths.gate_summary_path.exists());

    fs::remove_file(symlink_parent).expect("remove output root parent symlink");
    fs::remove_dir_all(symlink_target).expect("remove output root parent symlink target");
}

#[test]
fn output_root_writer_rejects_parent_file_before_partial_write() {
    let registry_toml =
        fs::read_to_string(repo_root().join("specs/sources.toml")).expect("source registry");
    let source_summary = summarize_source_registry_toml(&registry_toml).expect("source summary");
    let report = build_differential_report_with_source_context(
    ...
    let parent_file = std::env::temp_dir().join(format!(
        "ootd-differential-output-root-parent-file-{unique_suffix}"
    ));
    fs::write(&parent_file, b"not a directory").expect("output root parent file");
    let output_root = parent_file.join("artifacts");

    let error =
        write_differential_report_and_gate_to_output_root(&report, &source_summary, &output_root)
            .expect_err("output root parent file should fail before write");
    ...
    fs::remove_file(parent_file).expect("remove output root parent file");
}
...
#[cfg(unix)]
#[test]
fn output_root_loader_rejects_parent_symlink_as_contract_error() {
    let registry_toml =
        fs::read_to_string(repo_root().join("specs/sources.toml")).expect("source registry");
    let source_summary = summarize_source_registry_toml(&registry_toml).expect("source summary");
    let report = build_differential_report_with_source_context(
    ...
    let unique_suffix = SystemTime::now()
        ...
        .as_nanos();
    let symlink_target = std::env::temp_dir().join(format!(
        "ootd-differential-output-root-load-parent-symlink-target-{unique_suffix}"
    ));
    let symlink_parent = std::env::temp_dir().join(format!(
        "ootd-differential-output-root-load-parent-symlink-{unique_suffix}"
    ));
    let target_output_root = symlink_target.join("artifacts");
    write_differential_report_and_gate_to_output_root(
        &report,
        &source_summary,
        &target_output_root,
    )
    .expect("write target output root");
    std::os::unix::fs::symlink(&symlink_target, &symlink_parent)
        .expect("output root parent symlink");
    let output_root = symlink_parent.join("artifacts");

    let error = load_differential_artifacts_from_output_root(&output_root, &source_summary)
        .expect_err("output root parent symlink should fail before import");
...
#[test]
fn output_root_loader_rejects_parent_file_as_contract_error() {
    let registry_toml =
        fs::read_to_string(repo_root().join("specs/sources.toml")).expect("source registry");
    let source_summary = summarize_source_registry_toml(&registry_toml).expect("source summary");
    let unique_suffix = SystemTime::now()
        ...
        .as_nanos();
    let parent_file = std::env::temp_dir().join(format!(
        "ootd-differential-output-root-load-parent-file-{unique_suffix}"
    ));
    fs::write(&parent_file, b"not a directory").expect("output root parent file");
    let output_root = parent_file.join("artifacts");

    let error = load_differential_artifacts_from_output_root(&output_root, &source_summary)
        .expect_err("output root parent file should fail before import");
...
fn output_root_loader_rejects_artifact_directory_as_contract_error() {
    ...
    let unique_suffix = SystemTime::now()
        ...
        .as_nanos();
    let output_root = std::env::temp_dir().join(format!(
        "ootd-differential-output-root-load-path-preflight-{unique_suffix}"
    ));
    let expected_paths = differential_artifact_paths(&output_root);
    fs::create_dir_all(&output_root).expect("output root");
    write_differential_report_to_path(&report, &expected_paths.report_path).expect("write report");
    fs::create_dir_all(&expected_paths.gate_summary_path).expect("gate path directory");

    let error = load_differential_artifacts_from_output_root(&output_root, &source_summary)
        .expect_err("artifact directory should fail before import");
...
fn loads_differential_gate_summary_and_rejects_stale_contract() {
    ...
    let overflow_non_blocking_status_count = gate_json
        .replace(
            r#""unsupportedCaseCount": 0"#,
            &format!(r#""unsupportedCaseCount": {}"#, usize::MAX),
        )
        .replace(r#""skippedCaseCount": 0"#, r#""skippedCaseCount": 1"#);
    let error = load_differential_gate_from_json(&overflow_non_blocking_status_count)
        .expect_err("overflowing non-blocking status count should fail");
    match error {
        DifferentialReportLoadError::Contract { message } => {
            assert!(message.contains("non-blocking status counts overflowed"));
        }
        other => panic!("unexpected error: {other:?}"),
    }
...
fn differential_report_rejects_duplicate_case_names() {
    ...
    let error = load_differential_report_from_json(&json)
        .expect_err("duplicate case name should fail load");
    match error {
        DifferentialReportLoadError::Contract { message } => {
            assert!(message.contains("duplicate case name Application.Version"));
        }
        other => panic!("unexpected error: {other:?}"),
    }
    ...
    let unique_suffix = SystemTime::now()
        ...
        .as_nanos();
    let path = std::env::temp_dir().join(format!(
        "ootd-differential-report-duplicate-{unique_suffix}.json"
    ));
    let write_error = write_differential_report_to_path(&report, &path)
        .expect_err("duplicate case name should fail write");
    match write_error {
        DifferentialReportLoadError::Contract { message } => {
            assert!(message.contains("duplicate case name Application.Version"));
        }
...
fn differential_report_rejects_empty_report_metadata() {
    for (library, version, profile, expected_message) in [
        ...
        let error = load_differential_report_from_json(&json)
            .expect_err("empty report metadata should fail load");
        match error {
            DifferentialReportLoadError::Contract { message } => {
                assert!(
                    message.contains(expected_message),
        ...
        let unique_suffix = SystemTime::now()
            ...
            .as_nanos();
        let path = std::env::temp_dir().join(format!(
            "ootd-differential-report-empty-metadata-{unique_suffix}.json"
        ));
        let write_error = write_differential_report_to_path(&report, &path)
            .expect_err("empty report metadata should fail write");
        match write_error {
            DifferentialReportLoadError::Contract { message } => {
                assert!(
                    message.contains(expected_message),
...
fn differential_report_rejects_invalid_case_artifact_references() {
    for (artifact_key, artifact_path, expected_message) in [
        ...
        (
            "runtimeTrace",
            "../runtime.json",
            "artifact runtimeTrace path ../runtime.json must be relative",
        ),
        (
            "runtimeTrace",
            ".",
            "artifact runtimeTrace path . must be relative",
        ),
        (
            "runtimeTrace",
            "./runtime.json",
        ...
        (
            "runtimeTrace",
            "reports/runtime:trace.json",
            "artifact runtimeTrace path reports/runtime:trace.json contained reserved portable path character",
        ),
        (
            "runtimeTrace",
            "reports/runtime?.json",
            "artifact runtimeTrace path reports/runtime?.json contained reserved portable path character",
        ),
        (
            "runtimeTrace",
            "reports/runtime\u{7}.json",
            "artifact runtimeTrace path reports/runtime\u{7}.json contained reserved portable path character",
        ),
        (
            "runtimeTrace",
            "reports/CON.json",
        ...
        let report = build_differential_report(
            ...
            vec![DifferentialCaseResult {
                ...
                actual: Some("1".to_string()),
                message: Some("runtime collapsed the reference to the first scalar".to_string()),
                artifacts: BTreeMap::from([(artifact_key.to_string(), artifact_path.to_string())]),
            }],
        );
        let json = serde_json::to_string(&report).expect("report json");

        let load_error = load_differential_report_from_json(&json)
            .expect_err("invalid artifact reference should fail load");
        match load_error {
            DifferentialReportLoadError::Contract { message } => {
                assert!(
                    message.contains(expected_message),
        ...
        let unique_suffix = SystemTime::now()
            ...
            .as_nanos();
        let path = std::env::temp_dir().join(format!(
            "ootd-differential-report-invalid-artifact-{unique_suffix}.json"
        ));
        let write_error = write_differential_report_to_path(&report, &path)
...
#[test]
fn direct_differential_loaders_reject_directory_paths() {
    let unique_suffix = SystemTime::now()
        .duration_since(UNIX_EPOCH)
        .expect("system time")
        .as_nanos();

    for (path, load_error) in [
        {
            let path = std::env::temp_dir().join(format!(
                "ootd-differential-report-direct-load-dir-{unique_suffix}"
            ));
            fs::create_dir_all(&path).expect("report loader directory path");
            let error = load_differential_report_from_path(&path)
                .expect_err("report directory path should fail load");
            (path, error)
        },
        {
            let path = std::env::temp_dir().join(format!(
                "ootd-differential-gate-direct-load-dir-{unique_suffix}"
            ));
            fs::create_dir_all(&path).expect("gate loader directory path");
            let error = load_differential_gate_from_path(&path)
                .expect_err("gate directory path should fail load");
            (path, error)
        },
    ] {
        match load_error {
...
#[cfg(unix)]
#[test]
fn direct_differential_loaders_reject_symlink_paths() {
    let report = build_differential_report(
        "Excel",
        "16.0",
        "excel_365",
    ...
    let report_target = std::env::temp_dir().join(format!(
        "ootd-differential-report-direct-load-symlink-target-{unique_suffix}.json"
    ));
    let report_path = std::env::temp_dir().join(format!(
        "ootd-differential-report-direct-load-symlink-{unique_suffix}.json"
    ));
    let gate_target = std::env::temp_dir().join(format!(
        "ootd-differential-gate-direct-load-symlink-target-{unique_suffix}.json"
    ));
    let gate_path = std::env::temp_dir().join(format!(
        "ootd-differential-gate-direct-load-symlink-{unique_suffix}.json"
    ));
    fs::write(
        &report_target,
        serde_json::to_string(&report).expect("report json"),
    )
    .expect("report symlink target");
    fs::write(
        &gate_target,
        serde_json::to_string(&gate).expect("gate json"),
    )
    .expect("gate symlink target");
    ...
    for (path, load_error) in [
        (
            ...
            load_differential_report_from_path(&report_path)
                .expect_err("report symlink path should fail load"),
        ),
        (
            gate_path.as_path(),
            load_differential_gate_from_path(&gate_path)
                .expect_err("gate symlink path should fail load"),
        ),
    ] {
        match load_error {
            DifferentialReportLoadError::Contract { message } => {
...
#[test]
fn direct_differential_loaders_reject_parent_file_paths() {
    let unique_suffix = SystemTime::now()
        .duration_since(UNIX_EPOCH)
        .expect("system time")
        .as_nanos();
    let report_parent = std::env::temp_dir().join(format!(
        "ootd-differential-report-direct-load-parent-file-{unique_suffix}"
    ));
    let gate_parent = std::env::temp_dir().join(format!(
        "ootd-differential-gate-direct-load-parent-file-{unique_suffix}"
    ));
    fs::write(&report_parent, b"not a directory").expect("report parent file");
    fs::write(&gate_parent, b"not a directory").expect("gate parent file");
    let report_path = report_parent.join("differential_report.json");
    let gate_path = gate_parent.join("differential_gate_summary.json");

    for load_error in [
        load_differential_report_from_path(&report_path)
            .expect_err("report parent file path should fail load"),
        load_differential_gate_from_path(&gate_path)
            .expect_err("gate parent file path should fail load"),
    ] {
        match load_error {
            DifferentialReportLoadError::Contract { message } => {
                assert!(message.contains("differential artifact parent path"));
    ...
    assert!(report_parent.is_file());
    assert!(gate_parent.is_file());

    fs::remove_file(report_parent).expect("remove report parent file");
    fs::remove_file(gate_parent).expect("remove gate parent file");
}
...
#[test]
fn direct_differential_writers_reject_parent_file_paths() {
    let report = build_differential_report(
        "Excel",
        "16.0",
        "excel_365",
    ...
    let unique_suffix = SystemTime::now()
        ...
        .as_nanos();
    let report_parent = std::env::temp_dir().join(format!(
        "ootd-differential-report-direct-write-parent-file-{unique_suffix}"
    ));
    let gate_parent = std::env::temp_dir().join(format!(
        "ootd-differential-gate-direct-write-parent-file-{unique_suffix}"
    ));
    fs::write(&report_parent, b"not a directory").expect("report parent file");
    fs::write(&gate_parent, b"not a directory").expect("gate parent file");
    let report_path = report_parent.join("differential_report.json");
    ...
    for write_error in [
        write_differential_report_to_path(&report, &report_path)
            .expect_err("report parent file path should fail write"),
        write_differential_gate_to_path(&gate, &gate_path)
            .expect_err("gate parent file path should fail write"),
    ] {
        match write_error {
            DifferentialReportLoadError::Contract { message } => {
                assert!(message.contains("differential artifact parent path"));
