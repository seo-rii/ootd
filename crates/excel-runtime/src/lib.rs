...
mod tests {
    ...
    fn application_display_scroll_bars_dispatch_roundtrips_and_rejects_invalid_values() {
        ...
        assert!(expect_bool(
            runtime
                .dispatch_get(application, "DisplayScrollBars", &[])
                .expect("Application.DisplayScrollBars default")
        ));

        runtime
            .dispatch_set(application, "DisplayScrollBars", OmValue::Bool(false), &[])
            .expect("Application.DisplayScrollBars = false");
        assert!(!expect_bool(
            runtime
                .dispatch_get(application, "DisplayScrollBars", &[])
                .expect("Application.DisplayScrollBars after false")
        ));

        runtime
            .dispatch_set(application, "DisplayScrollBars", OmValue::Bool(true), &[])
            .expect("Application.DisplayScrollBars = true");
        assert!(expect_bool(
            runtime
                .dispatch_get(application, "DisplayScrollBars", &[])
                .expect("Application.DisplayScrollBars after true")
        ));
        ...
        assert_eq!(
            runtime
                ...
                .expect_err("Application.DisplayScrollBars should reject non-bool values")
                .code,
            OmErrorCode::TypeMismatch
        );
        assert_eq!(
    ...
    fn application_interactive_dispatch_roundtrips_and_rejects_invalid_values() {
        ...
        assert!(expect_bool(
            runtime
                .dispatch_get(application, "Interactive", &[])
                .expect("Application.Interactive default")
        ));

        runtime
            .dispatch_set(application, "Interactive", OmValue::Bool(false), &[])
            .expect("Application.Interactive = false");
        assert!(!expect_bool(
            runtime
                .dispatch_get(application, "Interactive", &[])
                .expect("Application.Interactive after false")
        ));

        runtime
            .dispatch_set(application, "Interactive", OmValue::Bool(true), &[])
            .expect("Application.Interactive = true");
        assert!(expect_bool(
            runtime
                .dispatch_get(application, "Interactive", &[])
                .expect("Application.Interactive after true")
        ));
        ...
        assert_eq!(
            runtime
                ...
                .expect_err("Application.Interactive should reject non-bool values")
                .code,
            OmErrorCode::TypeMismatch
        );
        assert_eq!(
            runtime
                .dispatch_get(application, "Interactive", &[OmValue::Bool(true)])
                .expect_err("Application.Interactive should reject index args")
                .code,
            OmErrorCode::InvalidArgument
        );
    }
    ...
    fn range_dispatch_get_and_set_reject_index_arguments() {
        ...
        assert_eq!(
            runtime
                .dispatch_get(range, "Formula2", &[OmValue::Number(1.0)])
                .expect_err("Range.Formula2 args")
                .code,
            OmErrorCode::InvalidArgument
        );
        assert_eq!(
            runtime
                .dispatch_get(range, "Formula2R1C1", &[OmValue::Number(1.0)])
                .expect_err("Range.Formula2R1C1 args")
                .code,
            OmErrorCode::InvalidArgument
        );
        assert_eq!(
        ...
        assert_eq!(
            runtime
                .dispatch_get(range, "Formula2Local", &[OmValue::Number(1.0)])
                .expect_err("Range.Formula2Local args")
                .code,
            OmErrorCode::InvalidArgument
        );
        assert_eq!(
            runtime
                .dispatch_get(range, "Formula2R1C1Local", &[OmValue::Number(1.0)])
                .expect_err("Range.Formula2R1C1Local args")
                .code,
            OmErrorCode::InvalidArgument
        );
        assert_eq!(
        ...
        assert_eq!(
            runtime
                ...
                .expect_err("Range.Formula2 set args")
                .code,
            OmErrorCode::InvalidArgument
        );
        assert_eq!(
        ...
        assert_eq!(
            runtime
                ...
                .expect_err("Range.Formula2Local set args")
                .code,
            OmErrorCode::InvalidArgument
        );
        assert_eq!(
            runtime
                ...
                .expect_err("Range.Formula2R1C1Local set args")
                .code,
            OmErrorCode::InvalidArgument
        );
    }
    ...
    #[test]
    fn range_dispatch_formula2_aliases_formula_storage_paths() {
        let mut runtime = ExcelRuntime::new();
        let workbook = runtime
            .open_workbook(OpenWorkbookSpec {
                bytes: synthetic_workbook_bytes(),
        ...
        runtime
            ...
            .expect("set C3 Formula2");
        assert_eq!(
            expect_text(
                runtime
                    .dispatch_get(c3, "Formula2", &[])
                    .expect("C3 Formula2")
            ),
            "=SUM(A1:B1)"
        );
        let c3_formula = runtime
            .workbook_state(workbook)
            ...
            .expect("C3 formula")
            .clone();
        assert_eq!(c3_formula.text, "SUM(A1:B1)");
        assert!(!c3_formula.is_r1c1);

        runtime
            .dispatch_set(
                d3,
                "Formula2R1C1",
                OmValue::Text("=RC[-1]*2".to_string()),
                &[],
            )
            .expect("set D3 Formula2R1C1");
        assert_eq!(
            expect_text(
                runtime
                    .dispatch_get(d3, "Formula2R1C1", &[])
                    .expect("D3 Formula2R1C1")
            ),
            "=RC[-1]*2"
        );
        let d3_formula = runtime
            .workbook_state(workbook)
            .expect("workbook state")
            .cell(sheet_id, 3, 4)
            .expect("D3")
            ...
            .expect("D3 formula")
            .clone();
        assert_eq!(d3_formula.text, "RC[-1]*2");
        assert!(d3_formula.is_r1c1);
        assert!(!expect_bool(
            runtime
                .dispatch_get(workbook.0, "Saved", &[])
    ...
    #[test]
    fn range_dispatch_formula2_local_aliases_formula_storage_paths() {
        let mut runtime = ExcelRuntime::new();
        let workbook = runtime
            .open_workbook(OpenWorkbookSpec {
                bytes: synthetic_workbook_bytes(),
        ...
        let c5 = expect_object_handle(
            runtime
                .dispatch_invoke(active_sheet, "Range", &[OmValue::Text("C5".to_string())])
                .expect("Range(C5)"),
        );
        let d5 = expect_object_handle(
            runtime
                .dispatch_invoke(active_sheet, "Range", &[OmValue::Text("D5".to_string())])
                .expect("Range(D5)"),
        );
        ...
        runtime
            ...
            .expect("set C5 Formula2Local");
        assert_eq!(
            expect_text(
                runtime
                    .dispatch_get(c5, "Formula2Local", &[])
                    .expect("C5 Formula2Local")
            ),
            "=SUM(A1:B1)"
        );
        let c5_formula = runtime
            .workbook_state(workbook)
            .expect("workbook state")
            .cell(sheet_id, 5, 3)
            .expect("C5")
            ...
            .expect("C5 formula")
            .clone();
        assert_eq!(c5_formula.text, "SUM(A1:B1)");
        assert!(!c5_formula.is_r1c1);

        runtime
            .dispatch_set(
                d5,
                "Formula2R1C1Local",
                OmValue::Text("=RC[-1]*4".to_string()),
                &[],
            )
            .expect("set D5 Formula2R1C1Local");
        assert_eq!(
            expect_text(
                runtime
                    .dispatch_get(d5, "Formula2R1C1Local", &[])
                    .expect("D5 Formula2R1C1Local")
            ),
            "=RC[-1]*4"
        );
    ...
    fn range_clear_removes_values_formulas_and_style_shells_without_changing_selection() {
        ...
        assert_eq!(
            expect_text(
                runtime
                    .dispatch_get(selection, "Address", &[])
                    .expect("Selection address after Clear")
            ),
            "$A$1:$C$1"
        );
        assert_eq!(
            runtime
                .dispatch_get(range, "Text", &[])
                .expect("Range(A1:C1).Text after Clear"),
            OmValue::Text(String::new())
        );
        assert_eq!(
            runtime
                .dispatch_get(formula_cell, "Value", &[])
                .expect("B1 Value after Clear"),
            OmValue::Empty
        );
        assert_eq!(
            runtime
                .dispatch_get(formula_cell, "Formula", &[])
                .expect("B1 Formula after Clear"),
            OmValue::Empty
        );
        assert_eq!(
            runtime
                .dispatch_get(formula_cell, "HasFormula", &[])
                .expect("B1 HasFormula after Clear"),
            OmValue::Bool(false)
        );
        let worksheet = runtime
            .runtime_workbook(workbook)
        ...
        assert!(!expect_bool(
            runtime
                .dispatch_get(workbook.0, "Saved", &[])
                .expect("Workbook.Saved after Clear")
        ));
        assert_eq!(
            runtime
                .dispatch_invoke(range, "Clear", &[OmValue::Bool(true)])
                .expect_err("Range.Clear args should be rejected")
                .code,
            OmErrorCode::InvalidArgument
        );
        ...
        let saved = runtime
            ...
            .expect("save workbook after Clear");
        let reopened = ExcelRuntime::new()
            .codec
            .load(&saved, LoadOptions::default())
            .expect("reload cleared workbook");
    ...
    fn range_fill_dispatch_rejects_arguments_and_read_only_workbooks() {
        ...
        assert_eq!(
            read_only_runtime
                .dispatch_invoke(read_only_range, "FillDown", &[])
                .expect_err("Range.FillDown should reject read-only workbooks")
                .code,
            OmErrorCode::InvalidState
        );
        assert_eq!(
