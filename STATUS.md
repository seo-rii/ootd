...
- Current root test inventory: 91 `excel-model`, 720 `excel-runtime`, and 2,920 `excel-xlsx`
  tests.
...
- Workbook-state save boundary: decoded and live states must retain unique bounded worksheet IDs,
  ...
  handle allocation and reload/save callers propagate failure. Worksheet collection and
  `WorksheetData` payload fields plus the remaining graph fields remain open under `OOTD-054`.
  Details are in
