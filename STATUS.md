...
| Workbook and worksheet model | Partial | Workbook, sheet, cell, name, chart, drawing, and basic dynamic-array state are modeled; `ExcelLimits` validates every supported range command and direct cell/spill mutation, while checked cardinality prevents `u32` area overflow; the worksheet-data ownership map is private, decoded construction is validated, and paired insert/remove plus existing-owner replacement commands prevent external orphan keys; model load/save preflight rejects empty collections, model-internal worksheet identity and ownership drift, partial/duplicate bindings, missing or orphan worksheet data, dangling local-name scope, and inconsistent spill topology, and workbook-ID reassignment validates every chart source before atomically committing all owned identities; post-materialization XLSX save matches each worksheet identity and binding against the discovered package graph; chart/drawing preflight rejects map/model/workbook/host drift, duplicate canonical raw-part ownership, broken sheet-to-drawing-to-chart relationships, and support-snapshot owner/inventory drift while preserving drawingless chart sheets, loaded shared charts, and normal source-snapshot invalidation subsets | Close the remaining worksheet collection, payload, and graph DTO fields through validated commands, then add Oracle-backed mutation and save/reopen cases |
...
- Current root test inventory: 90 `excel-model`, 720 `excel-runtime`, and 2,920 `excel-xlsx`
  tests.
...
- Workbook-state save boundary: decoded and live states must retain unique bounded worksheet IDs,
  ...
  coherent dynamic spill ownership. The worksheet-data map is private; codec construction passes a
  public parts DTO through full validation, runtime sheet lifecycle uses paired worksheet/data
  insert and remove commands, replacement requires an existing live owner, and chart
  materialization no longer invents default data. Cell/source-XML mutation and worksheet-data
  access reject unknown sheet IDs with `NotFound` and no state change. Malformed loaded local
  scopes fail parsing, and every violation is rejected before serialization or chart-graph
  materialization. Unbound chart-sheet records alone may reach the separate XLSX graph preflight.
  ...
  `WorksheetData` payload fields plus the remaining graph/model fields remain open under
  `OOTD-054`. Details are in
