...
| Behavioral Excel oracle | Partial | Typed cases, exact-byte run manifests, a bounded symlink-refusing suite/case/input/observation filesystem loader, a two-independent-run completeness/fingerprint/typed-drift gate, comparison/report bridge, `ExcelRuntime` adapter, .NET contract tests, and an isolated COM runner/watchdog exist; no real Excel observation is pinned | Assemble suite-wide Windows runs, execute the repeated-capture gate on a pinned Excel profile, then commit and replay the first required corpus |
