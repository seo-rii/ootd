...
- Current root test inventory: 99 `excel-model`, 730 `excel-runtime`, and 2,920 `excel-xlsx`
  tests.
...
- Workbook-state save boundary: decoded and live states must retain unique bounded worksheet IDs,
  ...
  `OOTD-054`; `Range.Clear`, style-only `ClearFormats`, and `Range.Replace` are the first payload
  mutations routed through spill-aware atomic model commands.
  Details are in
