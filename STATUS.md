...
| Formula2 and dynamic arrays | Partial | Seventeen array functions produce two-dimensional spill results; model value, A1/R1C1 formula families, `ClearContents`, and Fill source/destination commands reject spill topology edits atomically; worksheet array formula metadata restores and writes spill state across synthetic save/reopen; A1 `anchor#` resolves a materialized extent, and scalar dependents recalculate after dynamic materialization | Remaining runtime mutation paths, copied Formula2/group semantics, `@`, dynamic-to-dynamic dependency order/cycles, Excel-specific dynamic-array extension metadata, and Oracle agreement |
...
- Current root test inventory: 104 `excel-model`, 735 `excel-runtime`, and 2,920 `excel-xlsx`
  tests.
...
- Workbook-state save boundary: decoded and live states must retain unique bounded worksheet IDs,
  ...
  `OOTD-054`; `Range.Clear`, style-only `ClearFormats`, `Range.Replace`, `Range.Sort`, and all four
  `Range.Fill` directions are the first payload mutations routed through spill-aware atomic model
  commands.
  Details are in
...
- CI portability: Ubuntu Rust 1.94, Ubuntu MSRV Rust 1.88, and Windows Rust 1.94 run as independent
  test lanes. A bounded rustfmt gate covers 54 tracked files with four guarded monolith exceptions;
