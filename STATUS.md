...
- Current root test inventory: 720 `excel-runtime` tests and 2,920 `excel-xlsx` tests.
...
- Workbook-state save boundary: decoded and live states must retain unique bounded worksheet IDs,
  ...
  internal targets; normal drawing-source invalidation may retain inventory-only entries. Public
  field encapsulation remains open under `OOTD-054`. Details are in
