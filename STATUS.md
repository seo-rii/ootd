...
| Behavioral Excel oracle | Partial | Typed cases, exact-byte run manifests, a bounded symlink-refusing suite/case/input/observation and case-fragment loader, deterministic complete-run assembly with canonical observation paths, fresh-root atomic materialization, a cross-platform fragment assembly CLI with JSON receipt, a two-independent-run completeness/fingerprint/typed-drift gate, comparison/report bridge, `ExcelRuntime` adapter, .NET contract tests, and an isolated COM runner/watchdog exist; no real Excel observation is pinned | Add the Windows command that iterates the watchdog and feeds complete fragments to the assembly CLI, execute the repeated-capture gate on a pinned Excel profile, then commit and replay the first required corpus |
...
- Behavioral Oracle foundation: Rust and .NET contracts, runtime adapter, differential gate bridge,
  COM runner, watchdog, and the bounded fragment assembly/publish CLI are implemented and
  synthetic/fake-backed tests pass. Windows suite iteration remains open.
