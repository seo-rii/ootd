...
- Current root test inventory: 110 `excel-model`, 744 `excel-runtime`, and 2,920 `excel-xlsx`
  tests.
...
- Workbook-state save boundary: decoded and live states must retain unique bounded worksheet IDs,
  ...
  `Range.Fill` directions, `Range.Copy Destination`, and transactional `Range.Cut Destination` are
  the first payload mutations routed through spill-aware atomic model commands.
  Details are in
...
- CI portability: Ubuntu Rust 1.94, Ubuntu MSRV Rust 1.88, and Windows Rust 1.94 run as independent
  test lanes. A bounded rustfmt gate covers 56 tracked files with four guarded monolith exceptions;
