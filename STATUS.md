...
| Behavioral Excel oracle | Partial | Typed cases, exact-byte run manifests, a bounded symlink-refusing suite/case/input/observation filesystem loader, comparison/gate bridge, `ExcelRuntime` adapter, .NET contract tests, and an isolated COM runner/watchdog exist; no real Excel observation is pinned | Require two matching complete runs on the pinned Windows/Excel profile, then commit and replay the first required corpus |
