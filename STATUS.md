...
| Behavioral Excel oracle | Partial | Typed cases, exact-byte run manifests, a bounded symlink-refusing suite/case/input/observation and case-fragment loader, deterministic complete-run assembly with canonical observation paths, a two-independent-run completeness/fingerprint/typed-drift gate, comparison/report bridge, `ExcelRuntime` adapter, .NET contract tests, and an isolated COM runner/watchdog exist; no real Excel observation is pinned | Atomically persist and orchestrate suite-wide Windows runs, execute the repeated-capture gate on a pinned Excel profile, then commit and replay the first required corpus |
