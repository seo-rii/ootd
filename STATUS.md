...
| Charts and drawings | Partial | Typed chart mutation and lossless-first relationship graph lifecycle cover a broad surface; `Chart.Paste` rejects selectors with no modeled format/metadata result before touching chart, workbook, object-handle, or clipboard session state, values paste uses a destination transaction so late validation failure restores the complete pre-call state, and internal source/series dispatch handles do not survive successful or failed calls | Close the remaining `Chart.Paste` Cut and partial-format contracts (`BUG-008`~`BUG-009`), then remain feature-frozen until PivotChart work |
...
- Current root test inventory: 112 `excel-model`, 767 `excel-runtime`, and 2,920 `excel-xlsx`
  tests.
