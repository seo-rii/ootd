...
- Current root test inventory: 86 `excel-model`, 720 `excel-runtime`, and 2,920 `excel-xlsx`
  tests.
...
- Workbook-state save boundary: decoded and live states must retain unique bounded worksheet IDs,
  ...
  coherent dynamic spill ownership. Cell/source-XML mutation and worksheet-data access reject
  unknown sheet IDs with `NotFound` and no state change; save preflight catches orphan entries
  introduced through still-public fields with `InvalidState`. Malformed loaded local scopes fail
  parsing, and every violation is rejected before serialization or chart-graph materialization.
  Unbound chart-sheet records alone may reach the separate XLSX graph preflight.
  ...
  handle allocation and reload/save callers propagate failure. Public field encapsulation remains
  open under `OOTD-054`. Details are in
