...
- Current root test inventory: 95 `excel-model`, 723 `excel-runtime`, and 2,920 `excel-xlsx`
  tests.
...
- Workbook-state save boundary: decoded and live states must retain unique bounded worksheet IDs,
  ...
  mutations, so late failure is atomic. Chart-sheet delete, target-less copy registration,
  `WorksheetData` payload fields, and remaining graph fields remain open under `OOTD-054`.
  Details are in
