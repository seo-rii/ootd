...
- Current root test inventory: 116 `excel-model`, 773 `excel-runtime`, and 2,923 `excel-xlsx`
  tests.
...
- Workbook-state save boundary: decoded and live states must retain unique bounded worksheet IDs,
  ...
  merged-cell ranges fail closed only when they intersect the exact shift corridor, while tables,
  validation, charts, drawings, and raw worksheet metadata remain outside this bounded command.
  Details are in
