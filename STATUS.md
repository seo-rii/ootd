...
- Current root test inventory: 95 `excel-model`, 722 `excel-runtime`, and 2,920 `excel-xlsx`
  tests.
...
- Workbook-state save boundary: decoded and live states must retain unique bounded worksheet IDs,
  ...
  on failure. Chart-sheet add/delete, target-less copy registration, `WorksheetData` payload fields,
  and remaining graph fields remain open under `OOTD-054`.
  Details are in
