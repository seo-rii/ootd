...
| Workbook and worksheet model | Partial | Workbook, sheet, cell, name, chart, drawing, and basic dynamic-array state are modeled; `ExcelLimits` validates every supported range command and direct cell/spill mutation, while checked cardinality prevents `u32` area overflow; model load/save preflight rejects empty collections, model-internal worksheet identity and ownership drift, partial/duplicate bindings, missing worksheet data, dangling local-name scope, and inconsistent spill topology; post-materialization XLSX save matches each worksheet identity and binding against the discovered package graph; chart/drawing preflight rejects map/model/workbook/host drift, duplicate canonical raw-part ownership, broken sheet-to-drawing-to-chart relationships, and support-snapshot owner/inventory drift while preserving drawingless chart sheets, loaded shared charts, and normal source-snapshot invalidation subsets | Close raw public DTO fields through validated commands, prevent orphan worksheet data, then add Oracle-backed mutation and save/reopen cases |
...
- Workbook-state save boundary: decoded and live states must retain unique bounded worksheet IDs,
  ...
  bindings, worksheet data, resolvable local-name scopes, and coherent dynamic spill ownership.
  Malformed loaded local scopes fail parsing; violations introduced through public fields fail
  ...
  rejects canonical target aliases. The chart/drawing graph boundary additionally requires stable
  map/model/workbook/host identity, one typed owner per canonical raw part, and resolved
  sheet-to-drawing-to-chart relationships; package-bound drawingless chart sheets and shared loaded
  charts retain preserve-only behavior. Loaded worksheet/drawing/chart support snapshots must also
  retain live sheet keys, exact host and relationship owners, canonical part inventories, coherent
  source/summary maps, and declared internal targets; normal drawing-source invalidation may retain
  inventory-only entries. Orphan worksheet data and field encapsulation remain open. Details are in
