...
| Charts and drawings | Partial | Typed chart mutation and lossless-first relationship graph lifecycle cover a broad surface; `Chart.Paste` rejects selectors with no modeled format/metadata result before touching chart, workbook, object-handle, or clipboard session state, and values paste uses a destination transaction so late validation failure restores the complete pre-call state | Close the remaining `Chart.Paste` internal-handle lifecycle, Cut, and partial-format contracts (`BUG-007`~`BUG-009`), then remain feature-frozen until PivotChart work |
...
- Current root test inventory: 112 `excel-model`, 764 `excel-runtime`, and 2,920 `excel-xlsx`
  tests.
...
- CI portability: Ubuntu Rust 1.94, Ubuntu MSRV Rust 1.88, and Windows Rust 1.94 run as independent
  test lanes. A bounded rustfmt gate covers 60 tracked files with four guarded monolith exceptions;
