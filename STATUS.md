...
- Current root test inventory: 96 `excel-model`, 727 `excel-runtime`, and 2,920 `excel-xlsx`
  tests.
...
- Workbook-state save boundary: decoded and live states must retain unique bounded worksheet IDs,
  ...
  `OOTD-054`; `Range.Clear` is the first payload mutation routed through a spill-aware atomic model
  command.
  Details are in
